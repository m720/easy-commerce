  ...
  return (
    <div className="p-6 space-y-8">
      ...
      <div className="bg-white border border-zinc-200 rounded-xl shadow-sm shadow-zinc-900/5 p-5">
        <h2 className="text-base font-semibold text-zinc-800 mb-3">Date Range Filter</h2>
        <div className="flex flex-wrap items-end gap-4">
          ...
          {(fromDate || toDate) && (
            <button
              ...
              className="border px-3 py-2 rounded-lg text-sm hover:bg-zinc-50"
            >
              Clear
            </button>
          )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          ...
          <div key={card.label} className="bg-white border border-zinc-200 rounded-xl shadow-sm shadow-zinc-900/5 p-5 flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              {card.value === null ? (
                <div className="h-6 w-20 bg-zinc-200 animate-pulse rounded mt-1" />
              ) : (
      ...
      <div className="bg-white border border-zinc-200 rounded-xl shadow-sm shadow-zinc-900/5 p-5">
        ...
        {!topProducts ? (
          ...
          <p className="text-sm text-zinc-500 py-4 text-center">No product sales data yet.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-zinc-50 border-b">
                <tr>
                  <th className="px-3 py-2 text-left font-medium text-zinc-600">Product</th>
                  <th className="px-3 py-2 text-right font-medium text-zinc-600">Units Sold</th>
              ...
              <tbody>
                {topProducts.map((p, i) => (
                  <tr key={i} className="border-b last:border-0">
                    <td className="px-3 py-2 text-zinc-900">{p.product_name}</td>
                    <td className="px-3 py-2 text-right text-zinc-700">{p.total_sold}</td>
      ...
      <div className="bg-white border border-zinc-200 rounded-xl shadow-sm shadow-zinc-900/5 p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-zinc-900">Top Variants</h2>
          <button
        ...
        {!topVariants ? (
          ...
          <p className="text-sm text-zinc-500 py-4 text-center">No variant sales data yet.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-zinc-50 border-b">
              ...
              <tbody>
                {topVariants.map((v, i) => (
                  <tr key={i} className="border-b last:border-0">
                    <td className="px-3 py-2 text-zinc-900">{v.variant_name}</td>
                    <td className="px-3 py-2 text-right text-zinc-700">{v.total_sold}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="bg-white border border-zinc-200 rounded-xl shadow-sm shadow-zinc-900/5 p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-zinc-900">Coupon Usage</h2>
          <button
        ...
        {!coupons ? (
          ...
          <p className="text-sm text-zinc-500 py-4 text-center">No coupon usage data yet.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-zinc-50 border-b">
                <tr>
                  ...
                  <th className="px-3 py-2 text-right font-medium text-zinc-600">Used</th>
                  <th className="px-3 py-2 text-right font-medium text-zinc-600">Max Uses</th>
                </tr>
              </thead>
              <tbody>
                {coupons.map((c, i) => (
                  <tr key={i} className="border-b last:border-0">
                    <td className="px-3 py-2">
                      <span className="font-mono font-semibold text-xs bg-zinc-100 text-zinc-800 px-2 py-0.5 rounded">
                        {c.code}
                      </span>
                    </td>
                    <td className="px-3 py-2 text-right text-zinc-700">{c.used_count}</td>
                    <td className="px-3 py-2 text-right text-zinc-600">{c.max_uses ?? "∞"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <table className="w-full text-sm">
              <thead className="bg-zinc-50 border-b">
                <tr>
                  ...
                  <th className="px-3 py-2 text-right font-medium text-zinc-600">In Stock</th>
                  <th className="px-3 py-2 text-right font-medium text-zinc-600">Threshold</th>
                </tr>
              </thead>
              <tbody>
                {lowStock.map((item) => (
                  <tr key={item.variant_id} className="border-b last:border-0">
                    <td className="px-3 py-2 font-mono text-xs text-zinc-600">{item.sku}</td>
                    ...
                    <td className="px-3 py-2 text-right text-zinc-500">{item.low_stock_threshold}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="flex justify-end">
        <button
          ...
          className="inline-flex items-center gap-2 border px-4 py-2 rounded-lg text-sm font-medium hover:bg-zinc-50 transition-colors"
        >
