...
export default function ReturnsPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      <h1 className="text-2xl font-bold text-zinc-900">Return Requests</h1>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-zinc-50 border-b">
                <tr>
                  <th className="px-4 py-3 text-left font-medium text-zinc-600">Return ID</th>
                  <th className="px-4 py-3 text-left font-medium text-zinc-600">Order ID</th>
                  <th className="px-4 py-3 text-left font-medium text-zinc-600">Reason</th>
              ...
              <tbody>
                {returns.map((ret) => (
                  <>
                    <tr key={ret.id} className="border-b last:border-0 hover:bg-zinc-50">
                      ...
                      <td className="px-4 py-3 text-zinc-700 max-w-xs truncate">{ret.reason}</td>
                    ...
                    {actionRow?.id === ret.id && (
                      <tr key={`action-${ret.id}`} className="bg-zinc-50 border-b">
                        <td colSpan={6} className="px-4 py-3">
                          <div className="flex flex-col gap-2 max-w-lg">
                            <p className="text-sm font-medium text-zinc-700">
                              {actionRow.type === "approve" ? "Approving" : "Rejecting"} return —{" "}
                              <span className="text-zinc-500">add admin notes (optional)</span>
                            </p>
                            <textarea
                              value={notes}
                            ...
                            <div className="flex gap-2">
                              ...
                              <button
                                ...
                                className="px-4 py-1.5 text-sm border rounded hover:bg-zinc-100"
                              >
                                Cancel
                              </button>
                            </div>
