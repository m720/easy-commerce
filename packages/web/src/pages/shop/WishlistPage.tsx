          ...
          {wishlist.items.map((item) => {
            ...
            return (
              <div key={item.id} className="bg-white border rounded-xl p-4 flex gap-4">
                <Link to={`/products/${product.id}`} className="w-20 h-20 bg-gray-100 rounded-lg overflow-hidden flex-none">
                  {primaryImage?.url && (
                    <img src={primaryImage.url} alt={product.name} className="w-full h-full object-cover" />
                  )}
                </Link>
                <div className="flex-1 min-w-0">
                  <Link to={`/products/${product.id}`} className="font-medium text-gray-900 hover:text-blue-600 truncate block">
                    {product.name}
                  </Link>
                  <p className="text-sm font-semibold text-gray-900 mt-1">{formatPrice(product.base_price)}</p>
