  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
              {images.map((img, i) => (
                <button
                  ...
                  className={`flex-none w-16 h-16 rounded-lg overflow-hidden border-2 ${i === activeImage ? "border-blue-500" : "border-transparent"}`}
                >
        ...
        <div className="space-y-4">
          <div>
            {product.category && <p className="text-sm text-blue-600 font-medium">{product.category.name}</p>}
          ...
          {product.tags.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {product.tags.map((tag) => (
                <span key={tag.id} className="text-xs bg-gray-100 text-gray-600 px-2.5 py-1 rounded-full">{tag.name}</span>
              ))}
            </div>
          )}
          ...
          <div className="flex gap-3 pt-2">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 bg-blue-600 text-white py-3 rounded-xl font-medium hover:bg-blue-700 disabled:opacity-50 transition-colors"
            >
      ...
      <div className="mt-16">
        ...
        {token && (
          <form onSubmit={handleSubmitReview} className="bg-white border rounded-xl p-6 mb-8">
              ...
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((r) => (
                  <button
                    ...
                    className={`text-2xl transition-colors ${r <= reviewRating ? "text-yellow-400" : "text-gray-300"}`}
                  >
                    ★
                  </button>
                ))}
            ...
            <button
              ...
              className="mt-3 bg-blue-600 text-white px-6 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 disabled:opacity-60"
            >
