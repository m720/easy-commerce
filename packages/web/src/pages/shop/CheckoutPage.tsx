...
export default function CheckoutPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold text-charcoal mb-8">Checkout</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-sage/30 rounded-card shadow-soft p-6">
                ...
                {addresses.map((addr) => (
                  <label
                    ...
                    className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-colors ${
                      selectedAddressId === addr.id ? "border-brand bg-brand/10" : "border-sage/30 hover:border-sage/40"
                    }`}
                  >
                    <input
                      type="radio"
            ...
            {showAddAddress && (
              <form onSubmit={handleAddAddress} className="mt-4 grid grid-cols-2 gap-3">
                ...
                <button
                  type="submit"
                  className="col-span-2 bg-brand text-white py-2 rounded-lg text-sm hover:bg-brand/90"
                >
                  Save Address
                </button>
              </form>
          ...
          {/* Coupon */}
          <div className="bg-white border border-sage/30 rounded-card shadow-soft p-6">
            <h2 className="font-semibold text-charcoal mb-4">Coupon Code</h2>
            <CouponInput
              subtotal={subtotal}
        ...
        <div>
          <div className="bg-white border border-sage/30 rounded-card shadow-soft p-6 sticky top-24">
            ...
            <button
              ...
              className="w-full mt-6 flex items-center justify-center gap-2 bg-brand text-white py-3 rounded-xl font-medium hover:bg-brand/90 disabled:opacity-50 transition-colors"
            >
