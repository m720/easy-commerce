...
export default function ProfilePage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold text-zinc-900 mb-2">Account Settings</h1>
      <p className="text-zinc-500 text-sm mb-8">Member since {user?.created_at ? formatDate(user.created_at) : ""}</p>

      {/* Tabs */}
      <div className="flex gap-1 bg-zinc-100 p-1 rounded-xl mb-8 w-fit">
        {tabs.map(({ key, label }) => (
          <button
            key={key}
            onClick={() => setTab(key)}
            className={`px-5 py-2 rounded-lg text-sm font-medium transition-all ${
              tab === key ? "bg-white shadow-sm text-zinc-900" : "text-zinc-600 hover:text-zinc-800"
            }`}
          >
            {label}
          </button>
      ...
      {tab === "profile" && (
        <div className="bg-white border border-zinc-200 rounded-2xl shadow-sm shadow-zinc-900/5 p-6">
          <h2 className="font-semibold text-zinc-900 mb-6">Personal Information</h2>
          <form onSubmit={profileForm.handleSubmit(onProfileSubmit)} className="space-y-4">
            ...
            <button
              ...
              className="bg-zinc-900 text-white px-6 py-2 rounded-lg text-sm font-medium hover:bg-zinc-800 disabled:opacity-60"
            >
      ...
      {tab === "password" && (
        <div className="bg-white border border-zinc-200 rounded-2xl shadow-sm shadow-zinc-900/5 p-6">
          <h2 className="font-semibold text-zinc-900 mb-6">Change Password</h2>
      ...
      {tab === "addresses" && (
        <div className="space-y-4">
          {addresses?.map((addr) => (
            <div key={addr.id} className="bg-white border border-zinc-200 rounded-2xl shadow-sm shadow-zinc-900/5 p-5 flex items-start justify-between">
              <div className="flex items-start gap-3">
                <MapPin size={18} className="text-indigo-500 mt-0.5 flex-none" />
                <div>
                  {addr.label && <p className="font-medium text-zinc-900">{addr.label}</p>}
                  <p className="text-sm text-zinc-600">{addr.street}, {addr.city}</p>
                  <p className="text-sm text-zinc-600">{addr.country} {addr.postal_code}</p>
                  {addr.is_default && <span className="text-xs text-indigo-600 font-medium">Default</span>}
                </div>
              </div>
          ...
          {showAddForm && (
            <form onSubmit={handleAddAddress} className="bg-white border border-zinc-200 rounded-2xl shadow-sm shadow-zinc-900/5 p-5 grid grid-cols-2 gap-3">
              {[
              ...
              <button
                type="submit"
                className="col-span-2 bg-zinc-900 text-white py-2 rounded-lg text-sm font-medium hover:bg-zinc-800"
              >
                Save Address
              </button>
            </form>
