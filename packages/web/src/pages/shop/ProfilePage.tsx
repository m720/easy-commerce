...
export default function ProfilePage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      ...
      <p className="text-gray-500 text-sm mb-8">Member since {user?.created_at ? formatDate(user.created_at) : ""}</p>
      ...
      {tab === "addresses" && (
        <div className="space-y-4">
          {addresses?.map((addr) => (
            <div key={addr.id} className="bg-white border rounded-xl p-5 flex items-start justify-between">
              <div className="flex items-start gap-3">
                <MapPin size={18} className="text-blue-500 mt-0.5 flex-none" />
                <div>
                  {addr.label && <p className="font-medium text-gray-900">{addr.label}</p>}
                  <p className="text-sm text-gray-600">{addr.street}, {addr.city}</p>
                  <p className="text-sm text-gray-600">{addr.country} {addr.postal_code}</p>
                  {addr.is_default && <span className="text-xs text-blue-600 font-medium">Default</span>}
                </div>
              </div>
          ...
          {showAddForm && (
            <form onSubmit={handleAddAddress} className="bg-white border rounded-xl p-5 grid grid-cols-2 gap-3">
              {[
              ...
              <button
                type="submit"
                className="col-span-2 bg-blue-600 text-white py-2 rounded-lg text-sm font-medium hover:bg-blue-700"
              >
                Save Address
              </button>
            </form>
