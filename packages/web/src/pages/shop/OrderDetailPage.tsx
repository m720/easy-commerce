...
export default function OrderDetailPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <button onClick={() => navigate("/orders")} className="flex items-center gap-2 text-sm text-zinc-500 hover:text-zinc-700 mb-6">
        <ArrowLeft size={16} /> Back to Orders
      </button>

      <div className="bg-white border border-zinc-200 rounded-2xl shadow-sm shadow-zinc-900/5 p-6 mb-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            <h1 className="text-xl font-bold text-zinc-900">Order #{order.id.slice(0, 8)}</h1>
            <p className="text-sm text-zinc-500 mt-1">Placed {formatDate(order.created_at)}</p>
          </div>
          ...
          {order.items.map((item) => (
            <div key={item.id} className="flex justify-between items-start">
              <div>
                ...
                <p className="text-sm text-zinc-500">{item.variant_name} × {item.quantity}</p>
              </div>
        ...
        {order.shipping_address_snapshot && (
          <div className="border-t mt-4 pt-4">
            <p className="text-sm font-medium text-zinc-700 mb-1">Shipping to:</p>
      ...
      {showReturnForm && (
        <div className="bg-white border border-zinc-200 rounded-2xl shadow-sm shadow-zinc-900/5 p-6">
          <h2 className="font-semibold text-zinc-900 mb-4">Request Return</h2>
          <form onSubmit={handleReturn} className="space-y-4">
            ...
            <div>
              <p className="text-sm font-medium text-zinc-700 mb-2">Items to return:</p>
              {order.items.map((item) => (
                <div key={item.id} className="flex items-center justify-between py-2 border-b last:border-0">
                  <div>
                    <p className="text-sm">{item.product_name} — {item.variant_name}</p>
                    <p className="text-xs text-zinc-500">Ordered: {item.quantity}</p>
                  </div>
                  <input
                    type="number"
                    min={0}
